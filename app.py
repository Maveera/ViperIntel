...
st.markdown("""
    ...
    /* Fixed Single-Line Key Freeze Container */
    .key-freeze-row {
    ...
    /* Column-based Edit alignment fix */
    [data-testid="column"] {
        display: flex;
        align-items: center;
    }
...
with st.sidebar:
    ...
    # FIXED: Single-Line Alignment logic
...
if st.button("⚡ EXECUTE DEEP SCAN") and uploaded_file:
    if not any([st.session_state[f"{e}_key"] for e in engines]):
        st.error("❌ Please lock at least one API key in the sidebar.")
    else:
        ...
        for i, ip in enumerate(ips):
            ...
            # Forensic details from VirusTotal profile
            intel = {
...
if st.session_state.scan_results is not None:
    ...
    m2.markdown(f"<div class='metric-card'><b>Malicious</b><br><h2 style='color:#ff4b4b;'>{len(res[res['VT Hits'] > 0])}</h2></div>", unsafe_allow_html=True)
    m3.markdown(f"<div class='metric-card'><b>Clean</b><br><h2>{len(res[res['VT Hits'] == 0])}</h2></div>", unsafe_allow_html=True)

    # Map
    st.subheader("🌐 Geographic Threat Origin")
    m = folium.Map(location=[20, 0], zoom_start=2, tiles="CartoDB dark_matter")
    for _, r in res.iterrows():
        color = 'red' if r['VT Hits'] > 0 else '#00ffcc'
