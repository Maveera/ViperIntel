...
# Primary Engines and Extended Global Engines
primary_engines = ["AbuseIPDB", "VirusTotal", "AlienVault OTX"]
extended_engines = ["IPQualityScore", "ThreatFox", "Shodan", "GreyNoise", "CriminalIP", "Cisco Talos"]
...
st.markdown("""
    ...
    .locked-label { color: #00ffcc; font-weight: bold; font-size: 14px; margin-top: 5px;}
    </style>
...
with st.sidebar:
    ...
    # API Input Function with Inline Lock/Edit
    ...
    # Search & Discovery Dropdown for more Engines
    with st.expander("🔍 Search More Engines"):
        search_engine = st.selectbox("Select Threat Intelligence Provider", ["Select..."] + extended_engines)
        if search_engine != "Select...":
...
if st.button("⚡ EXECUTE DEEP SCAN") and uploaded_file:
    active_keys = [st.session_state[f"{e}_key"] for e in all_engines]
    if not any(active_keys):
        ...
        for i, ip in enumerate(ips):
            ...
            intel = {
                ...
                "ISP": "Unknown", "Network": "N/A", "ASN": "N/A", "Reputation": 0,
                "Lat": 20.0, "Lon": 0.0
            }
